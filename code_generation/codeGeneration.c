...
void gcCopyAddrToRegister(struct reg *r, int addr)
{
  char sign;
  manageCode();
  // printf("array: %d, %d\n", r->vector, equalTypes(r->type, lookupTypeInSymbolTable(5)));
  if (!isInFunction() || (r->vector && equalTypes(r->type, lookupTypeInSymbolTable(5))))
...
void gcWriteLogicalOperation(char *operation, struct reg *l, struct reg *r)
{
  manageCode();
...
{
  manageCode();
  writeInTmpFile("\t\tIF (%s%d) GT(R%d);\t\t # condición revertida\n", r->label, r->index, label);
}
